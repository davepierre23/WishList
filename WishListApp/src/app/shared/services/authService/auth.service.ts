...
export class AuthService {
  ...
  constructor(
    ...
    this.user$ = this.authenticater.authState.pipe(
      switchMap(user => {
          // Logged in
        if (user) {
          return this.afs.doc<Users>(`${this.dbName}/${user.uid}`).valueChanges();
        } else {
          // Logged out
          return of(null);
        }
  ...
  updateUserData(user:any, userForm:UserForm) {
    ...
    const userRef: AngularFirestoreDocument<Users> = this.afs.doc(`${this.dbName}/${user.uid}`);
  ...
  singUpAccount(userForm: UserForm){
    return this.authenticater.createUserWithEmailAndPassword(userForm.emailAddress, userForm.password)
      .then((result) => {
...
loginInAccount(email :any, password :any){
  return this.authenticater.signInWithEmailAndPassword(email,password)
  .then((result) => {
    if(result.user){
      this.user$ =  this.afs.doc<Users>(`${this.dbName}/${result.user.uid}`).valueChanges();
      return true
    }else{
      window.alert(JSON.stringify(result))
      return false;
